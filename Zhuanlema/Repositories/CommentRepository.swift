...
class CommentRepository {
    ...
    func getComments(postId: String, limit: Int = 20, offset: Int = 0) async throws -> [Comment] {
        let token = userRepository.getCurrentAccessToken()
        return try await databaseService.getComments(postId: postId, limit: limit, offset: offset, accessToken: token)
    }
